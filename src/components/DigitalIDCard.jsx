...
export default function DigitalIDCard({ user, role, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden relative transform transition-all scale-100" onClick={e => e.stopPropagation()}>
        ...
        <div className={`h-32 ${currentRole.color} relative`}>
            ...
            <div className="absolute -bottom-12 left-1/2 -translate-x-1/2">
                {/* 傳入 user 讓頭像自動抓字 */}
                <UserAvatar user={user} size="xl" className="shadow-2xl border-4 border-white"/>
            </div>
        </div>
        ...
        <div className="pt-16 pb-8 px-6 text-center">
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 mb-6 text-left relative group">
                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-1">IronMedic UUID</p>
            ...
            <p className="text-[10px] text-slate-400 mt-2 font-mono">DIGITAL IDENTITY VERIFIED</p>
        </div>
      </div>
    </div>
  )
