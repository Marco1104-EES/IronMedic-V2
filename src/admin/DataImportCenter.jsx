...
import { Upload, FileSpreadsheet, CheckCircle, ArrowRight, Save, Database, Settings, LayoutList, Merge, Plus, Target, AlertTriangle, UserCheck, XCircle, BrainCircuit, Trash2, Edit } from 'lucide-react'

// 🎯 系統目標欄位定義 (CRM 標準化分類)
const TARGET_FIELDS = [
    { group: '🟢 【A~O】基本與聯絡資料', options: [
        { key: 'full_name', label: '姓名(A) *必填' },
        { key: 'birthday', label: '出生年月日(B)' },
        { key: 'national_id', label: '身分證字號(C)' },
        { key: 'phone', label: '手機(D)' },
        { key: 'contact_email', label: 'e-mail(E) (聯絡信箱)' },
        { key: 'address', label: '通訊地址(F)' },
        { key: 'shirt_size', label: '賽事衣服(G)' },
        { key: 'emergency_name', label: '緊急聯繫人(H)' },
        { key: 'emergency_phone', label: '緊急聯繫人電話(I)' },
        { key: 'emergency_relation', label: '緊急聯繫人關係(J)' },
        { key: 'english_name', label: '英文名(K)' },
        { key: 'medical_license', label: '醫護證照繳交情況(L)' },
        { key: 'dietary_habit', label: '飲食(M)' },
        { key: 'resume_url', label: '醫鐵履歷網址(N)' },
        { key: 'badges', label: '成就徽章(O)' },
    ]},
    { group: '🔵 【P~AB】權限與醫療設定', options: [
        { key: 'role', label: '醫鐵權限(P)' },
        { key: 'is_current_member', label: '當年度會員(Q)' },
        { key: 'training_status', label: '會員訓練(R)' },
        { key: 'is_team_leader', label: '帶隊官(S)' },
        { key: 'is_new_member', label: '新人(T)' },
        { key: 'license_expiry', label: '醫護證照有效期(U)' },
        { key: 'shirt_expiry_25', label: '三鐵服期限-25(V)' },
        { key: 'shirt_expiry_26', label: '三鐵服期限-26(W)' },
        { key: 'is_vip', label: 'VIP(X)' },
        { key: 'email', label: '報名系統登入/WIX(Y) *系統帳號' },
        { key: 'blood_type', label: '血型(Z)' },
        { key: 'medical_history', label: '病史(AA)' },
        { key: 'is_blacklisted', label: '黑名單(AB)' },
    ]},
    { group: '🟣 【AC~AO】賽事與後勤數據', options: [
        { key: 'total_points', label: '積分(AC)' },
        { key: 'total_races', label: '場次(AD)' },
        { key: 'volunteer_hours', label: '時數(AE)' },
        { key: 'rank_level', label: '等級(AF)' },
        { key: 'line_id', label: 'LineID(AG)' },
        { key: 'fb_id', label: 'FB(AH)' },
        { key: 'ig_id', label: 'IG(AI)' },
        { key: 'admin_note', label: '備註(AJ)' },
        { key: 'shirt_receive_date', label: '領衣日(AK)' },
        { key: 'cert_send_date', label: '證書日(AL)' },
        { key: 'transport_pref', label: '交通(AM)' },
        { key: 'stay_pref', label: '住宿(AN)' },
        { key: 'family_count', label: '眷屬(AO)' },
    ]},
    { group: '⚙️ 【AP~BI】擴充資料欄位', options: Array.from({length: 20}, (_, i) => ({ 
        key: `ext_${String(i+1).padStart(2,'0')}`, 
        label: `Ext_${String(i+1).padStart(2,'0')} (備用欄位 ${i+1})` 
    }))}
]

const FLAT_TARGETS = TARGET_FIELDS.flatMap(g => g.options)
// 🧠 定義 LocalStorage 記憶金鑰
const MAPPING_MEMORY_KEY = 'ironmedic_mapping_memory'

export default function DataImportCenter() {
  const [mode, setMode] = useState('full') // 'full': 完整資料建檔 | 'patch': 特定欄位更新
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                // 修正剩下的 10%：加入 cellDates: true 解析 Excel 內建日期格式
                const wb = XLSX.read(bstr, { type: 'binary', cellDates: true });
                const wsName = wb.SheetNames[0];
                const ws = wb.Sheets[wsName];
                
                // 轉換時，將日期物件轉為 YYYY-MM-DD 字串
                const jsonData = XLSX.utils.sheet_to_json(ws, { 
                    defval: "",
                    raw: false, 
                    dateNF: 'yyyy-mm-dd' // 強制日期格式
                });
                resolve(jsonData);
  ...
  const handleModeSwitch = (newMode) => {
      ...
      setRawData([]); setRawHeaders([]); setPreviewData([]); setLogs([]); setFieldMapping({}); setMemoryFlags({});
  }
  ...
  const handleExecute = async () => {
      ...
      if (mode === 'patch') {
          ...
          for (let i = 0; i < cleanRows.length; i += BATCH) {
              ...
              } catch (err) { fail += chunk.length }
          }
          addLog(`完整資料建檔作業完成。成功: ${success} 筆，失敗: ${fail} 筆。`, success > 0 ? 'success' : 'error')
      }
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in text-slate-800">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
                <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                    <Database className="text-blue-600"/> 資料整合匯入中心 <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded font-bold border border-slate-200">System V8.0</span>
                </h2>
                <p className="text-slate-500 text-sm mt-1">
                    企業級資料處理模組。支援「完整資料整合」與「特定欄位更新」雙重作業模式。
                </p>
            </div>
            
            {/* 模式切換按鈕 (移除軍事感) */}
            <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-200">
                <button onClick={()=>handleModeSwitch('full')} className={`px-4 py-2 rounded-md font-bold text-sm flex items-center gap-2 transition-all ${mode==='full' ? 'bg-white shadow-sm border border-slate-200 text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
                    <Merge size={16}/> 完整資料整合 (新增/覆寫)
                </button>
                <button onClick={()=>handleModeSwitch('patch')} className={`px-4 py-2 rounded-md font-bold text-sm flex items-center gap-2 transition-all ${mode==='patch' ? 'bg-white shadow-sm border border-slate-200 text-amber-600' : 'text-slate-500 hover:text-slate-700'}`}>
                    <Edit size={16}/> 特定欄位更新 (局部修改)
                </button>
            </div>
          </div>
        ...
        <div className="space-y-6">
            {mode === 'patch' ? (
                <div className="border-2 border-dashed border-slate-300 rounded-2xl p-12 text-center hover:border-amber-400 bg-white transition-all cursor-pointer">
                    ...
                    <label htmlFor="upload-patch" className="cursor-pointer block">
                        <div className="w-16 h-16 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center mb-4 mx-auto border border-amber-100">
                            <Edit size={32}/>
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 mb-2">上傳「特定欄位更新」資料表</h3>
                        <p className="text-sm text-slate-500 mb-4">此模式僅會更新您指定的欄位（例如：年度會員狀態），不會影響人員的其他資料。</p>
            ...
            <div className="flex justify-center mt-6">
                <button 
                    ...
                    disabled={!fileMaster || processing}
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                    <h4 className="font-bold text-slate-800 flex items-center gap-2">
                        ...
                        {mode === 'patch' ? '步驟 2-2：選擇欲更新的資料欄位' : '欄位對應設定 (Data Mapping)'}
                    </h4>
                        ...
                        return (
                            <div key={header} className={`p-4 rounded-xl border transition-all relative ${isMapped ? 'border-blue-300 bg-white shadow-sm' : 'border-slate-200 bg-white opacity-70'}`}>
                                ...
                                {isFromMemory && isMapped && (
                                    <div className="absolute -top-2.5 right-3 bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-blue-200 flex items-center gap-1">
                                        <BrainCircuit size={10}/> 智慧載入
                                    </div>
                                )}
            ...
            <div className="flex flex-wrap items-center justify-between p-5 rounded-2xl shadow-sm bg-white border border-slate-200 gap-4">
                {mode === 'patch' ? (
                    <div className="flex gap-6 items-center">
                        <div className="bg-green-50 px-4 py-2 rounded-lg border border-green-100">
                            <div className="text-xs text-green-600 font-bold mb-0.5">🟢 比對成功 (可更新)</div>
                            <div className="text-2xl font-black text-green-700">{previewData.filter(r=>['perfect','resolved'].includes(r._status)).length}</div>
                        </div>
                        <div className="bg-amber-50 px-4 py-2 rounded-lg border border-amber-100">
                            <div className="text-xs text-amber-600 font-bold mb-0.5">🟡 多筆重複 (需手動確認)</div>
                            <div className="text-2xl font-black text-amber-700">{previewData.filter(r=>r._status==='duplicate').length}</div>
                        </div>
                        <div className="bg-slate-50 px-4 py-2 rounded-lg border border-slate-200">
                            <div className="text-xs text-slate-500 font-bold mb-0.5">⚪ 查無資料 (將略過)</div>
                            <div className="text-2xl font-black text-slate-600">{previewData.filter(r=>r._status==='not_found').length}</div>
                        </div>
                    </div>
                ) : (
                    <div className="flex gap-6 items-center">
                        <div className="bg-green-50 px-4 py-2 rounded-lg border border-green-100">
                            <div className="text-xs text-green-600 font-bold mb-0.5">🟢 資料格式完整</div>
                            <div className="text-2xl font-black text-green-700">{previewData.filter(r=>r._status==='valid').length}</div>
                        </div>
                        <div className="bg-red-50 px-4 py-2 rounded-lg border border-red-100">
                            <div className="text-xs text-red-600 font-bold mb-0.5">🔴 必填欄位缺漏</div>
                            <div className="text-2xl font-black text-red-700">{previewData.filter(r=>r._status==='invalid').length}</div>
                        </div>
                    </div>
                )}
                ...
                <div className="flex gap-3">
                    ...
                    <button 
                        ...
                        disabled={processing || previewData.filter(r=>mode==='patch' ? ['perfect','resolved'].includes(r._status) : r._status==='valid').length === 0}
                        className="px-8 py-2.5 rounded-xl font-bold text-white shadow-md flex items-center gap-2 disabled:opacity-50 bg-slate-800 hover:bg-slate-700 transition-colors"
                    >
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-4 bg-slate-50 border-b border-slate-200 font-bold text-slate-700 flex justify-between items-center">
                    <span className="flex items-center gap-2 text-sm">
                        <LayoutList size={16} className="text-slate-500"/> 匯入資料預覽清單 (前 100 筆)
                    </span>
                </div>
                            ...
                            <tr>
                                ...
                                {mode === 'patch' ? (
                                    <>
                                        ...
                                        {Object.values(fieldMapping).filter(Boolean).map(col => (
                                            <th key={col} className="p-4 border-b text-blue-600">更新: {FLAT_TARGETS.find(f=>f.key===col)?.label || col}</th>
                                        ))}
                                    </>
                                ) : (
                                    <>
                                        <th className="p-4 border-b">資料來源</th>
                                        {Object.values(fieldMapping).filter(Boolean).slice(0,10).map(col => (
                                            <th key={col} className="p-4 border-b text-slate-700">{FLAT_TARGETS.find(f=>f.key===col)?.label || col}</th>
                                        ))}
                                        {Object.values(fieldMapping).filter(Boolean).length > 10 && <th className="p-4 border-b text-slate-400">...其他欄位</th>}
                                    </>
                                )}
                            </tr>
                        </thead>
                                    ...
                                    {mode === 'patch' ? (
                                        <>
                                            <td className="p-4">
                                                ...
                                                {row._status === 'not_found' && <span className="text-slate-400 font-medium text-xs"><XCircle size={14} className="inline mr-1"/>略過</span>}
                                                {row._status === 'duplicate' && (
                                                    <div className="space-y-1">
                                                        ...
                                                        <select className="w-full p-2 border border-amber-300 rounded bg-white text-xs font-medium text-slate-700" onChange={(e) => resolveDuplicate(row._id, e.target.value)} defaultValue="">
                                                            ...
                                                            {row._duplicates.map(dup => <option key={dup.id} value={dup.id}>{dup.full_name} | {dup.phone || '無電話'}</option>)}
                                        ...
                                        <>
                                            ...
                                            {Object.values(fieldMapping).filter(Boolean).length > 10 && <td className="p-4 text-slate-300">...</td>}
                                        </>
                                    )}
                                </tr>
                            ))}
      ...
      <div className="bg-slate-900 rounded-xl p-4 h-48 overflow-hidden flex flex-col shadow-inner">
          <div className="text-slate-400 text-xs font-bold border-b border-slate-700 pb-2 mb-2 flex items-center gap-2">
              <Database size={14}/> 系統執行紀錄 (System Logs)
          </div>
