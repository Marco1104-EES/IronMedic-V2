...
import { 
  LayoutDashboard, FileText, Settings, LogOut, Upload, AlertTriangle, 
  Users, Home, Menu
} from 'lucide-react'

// 子頁面
import DashboardHome from './DashboardHome' // ✨ 新增引入
  ...
  return (
    <aside className="w-64 bg-[#0f172a] text-white flex flex-col fixed h-full z-20 shadow-2xl border-r border-slate-800">
        ...
        <div className="p-4 border-t border-slate-800 bg-[#020617]">
           <button onClick={() => onNavigate('/home')} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded-lg text-xs font-bold flex justify-center items-center transition-colors mb-2 border border-slate-700">
             <Home size={14} className="mr-2" /> 返回前台官網
           </button>
           <button onClick={onLogout} className="w-full text-red-400 hover:bg-red-900/20 hover:text-red-300 py-2 text-xs font-bold flex justify-center items-center rounded-lg transition-colors border border-transparent hover:border-red-900/30">
  ...
  return (
    <div className="flex min-h-screen bg-[#f1f5f9] font-sans">
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto h-screen custom-scrollbar">
        ...
        <header className="flex justify-between items-center mb-8 bg-white p-4 rounded-xl shadow-sm border border-slate-200 sticky top-0 z-10">
           <div className="flex items-center">
             ...
             <div>
               ...
               <div className="flex items-center text-[10px] text-slate-400 mt-1 font-mono font-bold uppercase">
           ...
           <div className="flex items-center gap-4">
             <div className="text-right mr-2 hidden sm:block">
               <div className={`text-[10px] font-black uppercase tracking-wider ${userRoleConfig.color}`}>
                 {userRoleConfig.rank}
               </div>
               <div className="text-sm font-bold text-slate-700">{userRoleConfig.label}</div>
             </div>
             <div className={`w-10 h-10 rounded-lg text-white flex items-center justify-center font-bold shadow-md ring-2 ring-white text-sm ${userRoleConfig.bg}`}>
        ...
        {/* 路由表 */}
        <Routes>
          <Route path="/" element={<DashboardHome />} /> {/* ✨ 復原：載入儀表板首頁 */}
